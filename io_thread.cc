...
void IOThread::Init() {
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  //after execution of Io thread i will call and instanciate the controller Deprecator

  std::ofstream myfile;
  myfile.open ("ioThread.txt",std::fstream::app);
  myfile << "I/O thread instanciate deprecator nullptr\n";
  DeprecatorClass *singleton=nullptr;
  myfile << singleton;
  myfile << "\n";
  myfile << "I/O thread ask an instance of the deprecator\n";
